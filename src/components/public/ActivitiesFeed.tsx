            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
              <div className="flex flex-wrap items-center gap-3 text-[11px] text-stone-400">
                <span className="inline-flex items-center gap-1">
                  <CalendarDays className="h-3 w-3" /> {formatDateHi(post.date)}
                </span>
                {post.location && (
                  <span className="inline-flex items-center gap-1">
      ...
      <button
        ...
        className={cn(
          ...
          active === ""
            ...
            : "border border-stone-200 bg-white text-stone-600 hover:border-saffron-300 hover:bg-saffron-50",
        )}
      >
        सभी
      </button>
...
export function ActivitiesFeed({
  ...
  // Separate featured (max 1) from regular — only when no active filter/search
